...
Interval `r maxInterval` contains the maximum number of steps with an average of `r maxSteps`.
...
There are `r missingRecords` missing recods in the data set.
...
```
The new mean number of steps taken per day is `r newMean` and the new median number of steps
take per day is `r newMedian`. The median goes down compared to the original calculation and matches the mean.
